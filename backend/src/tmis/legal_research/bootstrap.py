...
def get_research_orchestrator() -> ResearchOrchestrator:
    ...
    the LRE's own connectors (internal documentation, private database —
    real if configured, the Sprint 5 in-memory fixtures otherwise, see
    `tmis.legal_research.connectors.factory`) are registered onto the
    Kernel's existing `ConnectorManager` alongside the codes/
    jurisprudence/doctrine connectors, so agents keep querying through
    one Kernel.
    """
    kernel = get_kernel()
