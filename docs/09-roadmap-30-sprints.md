...
```mermaid
flowchart TB
    subgraph Phase1["Phase 1 — Socle (S1-S19)"]
        ...
        S18[S18 Identity & Firm]
        S19[S19 Billing & abonnements]
    end
    subgraph Phase2["Phase 2 — RAG & Recherche (S20-S22)"]
        S20[S20 Module Document + Persistance]
        S21[S21 RAG et connecteurs branchés sur données réelles]
        S22[S22 Cache Redis en production + reranker appris]
    end
    subgraph Phase3["Phase 3 — Agents IA (S23-S30)"]
        S23[S23 Intégration agents métier au Kernel + Agent Analyse]
        S24[S24 Agent Synthèse narrative]
        S25[S25 Agent Vérificateur]
        S26[S26 Chat IA]
        S27[S27 Agent Recherche Documentaire]
        S28[S28 Agent Jurisprudence]
        S29[S29 Module Contrats + Agent Contrat]
        S30[S30 Agent Veille]
    end
    subgraph Phase4["Phase 4 — Pilotage & Plateforme (S31-S33)"]
        S31[S31 Sécurité renforcée & RGPD]
        S32[S32 Facturation avancée — webhooks Stripe]
        S33[S33 API publique — webhooks sortants]
    end
    subgraph Phase5["Phase 5 — Qualité & Lancement (S34-S37)"]
        S34[S34 Performance & scalabilité]
        S35[S35 Observabilité complète]
        S36[S36 UX polish & accessibilité]
        S37[S37 Durcissement pré-lancement]
    end
...
| 18 | Identity & Firm | Authentification, multi-tenant, RBAC | `identity`, `firm` | OAuth2, MFA, gestion cabinet/utilisateurs, tests d'isolation tenant |
| 19 | Billing & abonnements — intégration Stripe réelle | Le mécanisme (plans/quotas/essai gratuit) est déjà livré par `tmis.cabinet_os.subscriptions` (Sprint 9) | `billing` | Intégration Stripe (mode test) derrière `PaymentGatewayPort` |
...
| 25 | Agent Vérificateur | Fiabilité des réponses (règles métier) | Vérification transverse | S'appuie sur `ReasoningOrchestrator`/`ConfidenceEngine`/`ConflictDetector` (Sprint 6) pour le marquage d'incertitude plutôt que de reconstruire un moteur de cohérence |
| 26 | Chat IA | Interface conversationnelle | `assistant` | Chat streaming, historique par dossier |
| 27 | Agent Recherche Documentaire | Intégration agent ↔ `ResearchOrchestrator` (Sprint 5) | `legal_research` | Recherche exposée dans le chat avec citations, via `TMISKernel` — aucune réimplémentation du LRE |
| 28 | Agent Jurisprudence | Recherche de décisions | Jurisprudence | Comparaison de solutions jurisprudentielles |
| 29 | Module Contrats | Analyse contractuelle | `contract` | Détection de risques, comparaison de versions — s'appuie sur `tmis.cabinet_knowledge.clauses`/`tmis.cabinet_knowledge.templates` (Sprint 12) plutôt que de redévelopper une bibliothèque de clauses ou de modèles distincte |
| 30 | Agent Veille | Veille juridique | `watch` | Alertes ciblées depuis sources configurées |
| 31 | Sécurité renforcée & RGPD | Conformité | Transverse | Droits RGPD, suppression sécurisée, audit trail complet — s'appuie sur `tmis.platform.compliance`/`tmis.platform.security` (Sprint 10) plutôt que de reconstruire ces briques |
| 32 | Facturation avancée — webhooks Stripe réels | Les quotas d'usage sont déjà suivis par `tmis.cabinet_os.subscriptions` (Sprint 9) | `billing` | Webhooks Stripe entrants (événements de paiement) |
| 33 | API publique — webhooks sortants | Clés API/OAuth2/scopes/rate limiting/versionnage déjà livrés par `tmis.cabinet_os.public_api` (Sprint 9) | Transverse | Webhooks sortants vers des intégrations clientes Entreprise |
| 34 | Performance & scalabilité | Tenue en charge | Transverse | Profiling, cache, tests de charge — s'appuie sur `tmis.platform.performance`/`tmis.platform.cache` (Sprint 10) plutôt que de reconstruire ces briques |
| 35 | Observabilité complète | Exploitation | Transverse | Traces, métriques, dashboards, alerting — branche un exportateur réel derrière `tmis.platform.monitoring`/`tmis.platform.metrics` (Sprint 10) plutôt que de reconstruire ces briques |
| 36 | UX polish & accessibilité | Qualité perçue | Frontend | Mode sombre, responsive, accessibilité WCAG |
| 37 | Durcissement pré-lancement | Mise en production | Transverse | Pentest, audit RGPD final, documentation, bêta pilote |
